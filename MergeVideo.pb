...
Procedure ScanFolder(folder.s)
  ...
  If dir
    While NextDirectoryEntry(dir)
      If DirectoryEntryType(dir) = #PB_DirectoryEntry_File : count + 1 : EndIf
    Wend
    FinishDirectory(dir)
  ...
  If dir
    While NextDirectoryEntry(dir)
      If DirectoryEntryType(dir) = #PB_DirectoryEntry_File : gFiles(idx) = DirectoryEntryName(dir) : idx + 1 : EndIf
    Wend
    FinishDirectory(dir)
  EndIf
...
Procedure MoveItemUp()
  ...
  gFiles(sel - 1) = tmp
  RefreshListNumbers()
  SetGadgetState(#LIST_VIDEOS, sel - 1)
EndProcedure
...
Procedure RemoveSelectedItem()
  ...
  RefreshListNumbers()
  If gFileCount > 0
    If sel >= gFileCount : sel = gFileCount - 1 : EndIf
    SetGadgetState(#LIST_VIDEOS, sel)
  EndIf
  If gFileCount = 0
...
Procedure DoMerge()
  ...
  If WriteBatFile(batFile, batCmd) = ""
    ...
    HideGadget(#BTN_CANCEL_MERGE, #True) : DeleteFile(listFile) : ProcedureReturn
  EndIf
